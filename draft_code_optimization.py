...
prompts = {
    ...
    "verification_prompt": "Please verify whether the output code obtained in the previous step meets the specified modification requirements and has not undergone any other irrelevant changes. If it is the case, output 1; otherwise, output 0. output_code : \n{}\n previous step: \n{content}\n" 
}
...
def verification(in_s, content, client, model):
    content = prompts["verification_prompt"].format(output_code=in_s, content=content)
    return eval(response(client, model, content))
...
def post_processing(client, model, func_code, c_test):
    ...
    if returncode != 0:
        ...
        for i in range(MAX_ITERS - 1):
            ...
            if returncode == 0:
                break
            output = response(client, model, content).replace("}\n```", "}\n").replace("```c\n", "")
        func_code = output if verification(output, content, client, model) else func_code
    ...
    #
    # # 2: repair_prompt
    content = prompts["repair_prompt"].format(func_code=func_code)
    output = response(client, model, content).replace("}\n```", "}\n").replace("```c\n", "")
    func_code = output if verification(output, content, client, model) else func_code
    flag_compile, flag_run = eval_compile_and_run(c_test_include + "\n" + func_code + "\n" + c_test)
    ...
    # 3: rename_variables
    content = prompts["rename_variable_names_prompt"] + "\n" + func_code + "\n"
    output = response(client, model, content).replace("}\n```", "}\n").replace("```c\n", "")
    func_code = output if verification(output, content, client, model) else func_code
    flag_compile, flag_run = eval_compile_and_run(c_test_include + "\n" + func_code + "\n" + c_test)
    ...
    # 4: add_comment
    content = prompts["add_comment_prompt"].format(func_code=func_code)
    output = response(client, model, content).replace("}\n```", "}\n").replace("```c\n", "")
    func_code = output if verification(output, content, client, model) else func_code
    flag_compile, flag_run = eval_compile_and_run(c_test_include + "\n" + func_code + "\n" + c_test)
